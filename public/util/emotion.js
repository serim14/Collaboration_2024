...
video.addEventListener("play", () => {
  ...
  setInterval(async () => {
    ...
    //console.log(DetectionsArray[0].expressions)
  }, 50);
});
...
function detectionsDraw(canvas, DetectionsArray) {
  ...
  var expression = DetectionsArray[0].expressions;
  
  max = Math.max(...Object.values(expression))
  maxIndex = Object.keys(expression).find(key => expression[key] === max)

  // if(maxIndex =="neutral"){
  //   const text = checkText(':)');
  //   document.getElementById("emotion-Content").innerHTML = text;
  // }
